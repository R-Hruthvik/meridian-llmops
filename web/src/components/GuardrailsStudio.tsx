      ...
      <div className="lg:col-span-7 space-y-5">
        <div className="bg-meridian-card border border-meridian-border rounded-2xl p-5 shadow-glass space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-meridian-muted">NeMo Policy Rules</span>
          </div>

          {/* Quick Presets */}
          <div>
            <label className="block text-[11px] font-medium text-meridian-muted mb-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {presets.map((p, idx) => (
                <button
                  ...
                  className="text-[11px] px-3 py-1.5 rounded-lg bg-meridian-bg border border-meridian-border hover:border-meridian-secondary/50 text-meridian-text hover:text-meridian-lavender transition-all flex items-center space-x-1.5"
                >
                  <Zap className="w-3 h-3 text-meridian-secondary" />
                  <span>{p.name}</span>
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full h-32 bg-meridian-bg border border-meridian-border rounded-xl p-3.5 text-xs text-meridian-text placeholder-meridian-muted outline-none focus:border-meridian-primary resize-none transition-all leading-relaxed"
            />
          </div>
        ...
        <div className="bg-meridian-card border border-meridian-border rounded-2xl p-5 shadow-glass space-y-4">
          ...
          {!result ? (
            <div className="text-center py-12 text-meridian-muted text-xs border border-dashed border-meridian-border rounded-xl">
              <Lock className="w-8 h-8 mx-auto mb-2 text-meridian-border" />
              <p>No evaluation performed yet.</p>
            ...
            <div className="space-y-4">
              ...
              >
                {result.action_taken === 'blocked' ? (
                  <AlertOctagon className="w-6 h-6 shrink-0 text-rose-400" />
                ) : result.action_taken === 'redacted' ? (
                ...
                <div>
                  ...
                  <p className="text-[11px] opacity-80 mt-0.5">
                    Action Taken: <span className="font-semibold uppercase">{result.action_taken}</span>
                  </p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {result.policy_violations.map((violation, idx) => (
                      <div
                        key={idx}
                        className="text-xs p-2.5 rounded-lg bg-meridian-bg border border-rose-500/30 text-rose-300"
                      >
                        {violation}
                      </div>
                    ))}
              ...
              {result.sanitized_text && (
                <div>
                  ...
                  <div className="p-3 rounded-xl bg-meridian-bg border border-meridian-border text-xs font-mono text-meridian-text whitespace-pre-wrap leading-relaxed">
                    {result.sanitized_text}
                  </div>
                </div>
              )}
